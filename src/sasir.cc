...
void degrade_fftmask2d(cfarray &x, bool degrade, bool reverse)
{
	if(!reverse)
	{
		...
		if(degrade) x *= cMask;
//io_write_ima_complex_f("frame_out2",xFrame);
	}
	else
	{
		if(degrade) x *= cMask;
...
// Input data
	...
	cData.alloc(Data.nx(),Data.ny(),0);
//cerr<<"data nx ny nz "<<Data.nx()<<","<<Data.ny()<<","<<Data.nz()<<endl;
	cRecons.alloc(Data.nx(),Data.ny(),0);
	if(fourier_in)
	{
		...
		cMask.alloc(Data.nx(),Data.ny(),0);
		float norm = 1.F/sqrt(float(Data.nx()*Data.ny()));
		int nx = Data.nx(),ny = Data.ny();
		// cMask = (cData!=0)
		for(int j=0;j<ny;j++) for(int i=0;i<nx;i++)
		{
			cData(i,j) = norm*complex_f(Data(i,j,0),-Data(i,j,1));
			cMask(i,j) = abs(Data(i,j,0)) > 1e-6; //( (Data(i,j,0) != 0) && (Data(i,j,1) != 0) ); 
		//	cerr<<","<<cMask(i,j);//(Data(i,j,0) != 0);
		}
		//cMask.init(complex_f(1.0,0));
		Data.alloc(Data.nx(),Data.ny(),0);//,1
